 ...
 * After migration, every task is guaranteed to have timeHour/timeMinute.
 */
export function formatTaskTime(task) {
  const h = task.timeHour ?? 0;
  const m = task.timeMinute ?? 0;
 ...
 * Uses crypto.randomUUID() — available in React Native 0.71+ and all modern engines.
 * Falls back to timestamp + random suffix only if crypto is somehow unavailable
 * (e.g. very old runtime or unusual web environment).
 */
